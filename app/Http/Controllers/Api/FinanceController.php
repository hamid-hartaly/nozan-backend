...
class FinanceController extends Controller
{
    ...
    public function recordInvoicePayment(Request $request, string $invoiceId): JsonResponse
    {
        ...
        $hasJobLinkedExtras = $invoiceModel->serviceJobs->isNotEmpty()
            && $invoiceModel->lineItems->contains(fn (InvoiceLineItem $item): bool => $item->item_type === 'extra');

        if ($hasJobLinkedExtras) {
            $allocatableJobBalance = (int) $invoiceModel->serviceJobs->sum(function (ServiceJob $job): int {
                return max($this->amountForJob($job) - (int) round((float) ($job->payment_received_iqd ?? 0)), 0);
            });

            abort_if(
                $amount > $allocatableJobBalance,
                422,
                'This invoice includes extra items. Record only up to the remaining job balance in one payment until mixed allocation support is added.',
            );
        }
